...
class KDTree
{
  ...
  public:
     ...
     * For example:
     *
     *     Point<3> target(1, 3, 5);             sum d^2
     *     Point<3> currentBest1(1, 3, 2);        9
     *     Point<3> possibleBest1(2, 4, 4);       3
     *     Point<3> currentBest2(1, 3, 6);        1
     *     Point<3> possibleBest2(2, 4, 4);       3
     *     Point<3> currentBest3(0, 2, 4);        3
     *     Point<3> possibleBest3(2, 4, 6);       3
  ...
  private:
     ...
     */
     void build_KDTree(KDTreeNode*& subroot, vector<Point<Dim>>& newPoints, int left, int right, int curdim);
    // vector<Point<Dim>> sort(const vector<Point<Dim>>& newPoints);

    Point<Dim> select( vector<Point<Dim>>& newPoints, int left, int right, int median, int dim);
    void clear(KDTreeNode*& subroot);
    void copy(KDTreeNode*& subroot, KDTreeNode* otherRoot);
    //Point<Dim> findcurBest(const Point<Dim>& query, KDTreeNode* subroot, int curdim, stack<KDTreeNode*>& s, stack<int>& child)const;
    bool distance_check_only(const Point<Dim>& target,
                                    const Point<Dim>& currentBest,
                                    const Point<Dim>& potential) const;

    //Point<Dim> updateCurBest(KDTreeNode* node, Point<Dim> curBest, const Point<Dim>& query) const;
    Point<Dim> findNeighbor(const Point<Dim>& query, std::stack<KDTreeNode*>& path, const Point<Dim> cBest) const;
    std::stack<typename KDTree<Dim>::KDTreeNode*> search(KDTreeNode* subroot, const Point<Dim>& query,
